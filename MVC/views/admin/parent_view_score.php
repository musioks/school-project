...
<table class="table ">
  ...
  elseif ($this->session->userdata('level')==4) { 
      $std_email = mysql_query("select s.email from student s, parent p where p.id_no=20001 AND p.id_no = s.parent_id ");

      $student_info	= mysql_query("select s.student_id, s.adm_no, s.fname, s.lname, s.dob, s.doa, s.gender, s.religion, s.address, c.class, t.stream_name, s.parent_id, s.email FROM student s, class c, stream t , parent p WHERE s.class_id = c.class_id AND s.stream_id = t.stream_id AND p.id_no=s.parent_id AND s.email=".$std_email['email']);
}
